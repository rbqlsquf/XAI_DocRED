...
class BeamSearchAttentionDecoder(nn.Module):
    def __init__(self, hidden_size, num_sent, topk=2):
    ...
    def forward(self, last_hidden, decoder_inputs, encoder_outputs, attention_scores, attention_mask,
                evidence_scores=None, evidence_sentence_index=None, is_training=True):
        '''
        ...
        if evidence_scores is not None:
            ...
            evidence_scores = torch.stack(tmp_evidence_scores).view(-1, )
            attention_scores = torch.stack(refine_attention_scores, 0).view(batch_size, -1, 1, self.num_sent).transpose(
                0, 1)
            ...
            evidence_sentences = torch.stack(evidence_sentences, 0).view(-1, )
...
class ElectraForQuestionAnsweringBeamSearch(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        # 추가한 부분
        self.classifier = nn.Linear(in_features=self.max_seq_length, out_features=2)
    ...
    def forward(
        ...
        attention_masks = div_term.masked_fill(div_term != 1e-10, 0).masked_fill(div_term == 1e-10, 1).squeeze(
            dim=2).bool()
        ...
        # 각 배치에 해당하는 첫번째 문장 즉, 질문 벡터만 모아서 초기 decoder input으로 설정, 평균 벡터로 설정한듯
        # !!! cls_output으로 초기 input 설정하라고 하심

        # decoder_input : [batch, 1, hidden]
        decoder_inputs = cls_outputs.unsqueeze(dim=1)

        # encoder_outputs: pbathcm max_sent, hidde]
        encoder_outputs = sentence_representation.unsqueeze(dim=1)
        attention_masks[:, 0] = 1
        mm = 1 - attention_masks
        mm = mm.unsqueeze(1).expand(-1, self.max_dec_len, -1)
        attention_masks = attention_masks.masked_fill(attention_masks == 1, -1e10).masked_fill(attention_masks == 0,
                                                                                               0).unsqueeze(1)

        # Evidence Path 수만큼 Expand
        ...
        encoder_outputs = encoder_outputs.repeat(1, self.num_samples, 1, 1).view(-1, self.max_sent_num,
                                                                                 self.hidden_size)
        ...
        for evidence_step in range(self.max_dec_len):
            decoder_inputs, last_hidden, evidence_sentences, attention_scores, attention_masks, evidence_scores = self.gru(
                last_hidden, decoder_inputs, encoder_outputs, attention_scores, attention_masks, evidence_scores,
                evidence_sentences)
        # evidence_vector : [hidden, batch*5]
        ...
        linear_classfier = self.classifier(sequence_output.transpose(1, 2))
        linear_classfier = linear_classfier.transpose(1, 2)
        # linear_classifier : [batch, 2, hidden]
        # label_logits : [batch, 2, hidden] * [batch, hidden, num_sent] = [batch, 2, num_sent]
        label_logits = linear_classfier.matmul(evidence_vector.view(-1, self.num_samples, batch_size).permute(2, 0, 1))
