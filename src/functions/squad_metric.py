...
def restore_prediction_2(results, features, n_best_size, tokenizer, max_answer_length=30):
...
def compute_predictions_logits(
    ...
    for example_index, examples in enumerate(all_examples):
        ...
        for pred in prelim_predictions:
            ...
            nbest.append(
                _NbestPrediction(
                    text=final_text, start_logit=pred.start_logit, end_logit=pred.end_logit, evidence=evidences
                )
            )
    ...
    for qas_id in all_predictions.keys():
        all_predictions[qas_id] = sorted(
            all_nbest_json[qas_id], key=lambda x: x["start_logit"] + x["end_logit"], reverse=True
        )[0]["text"]
...
def save_file_with_evidence(all_examples, all_features, all_results, output_prediction_file, tokenizer):
    ...
    for k, result in enumerate(all_results):
        ...
        evidence_indexs = result.evidence  # [batch, 3]
        label_logits = result.label_logits
        pred_labels = result.pred_labels
        unique_ids = result.unique_id
        ...
        for i, feature in enumerate(features):
            ...
            evidence_index = evidence_indexs[i]
            # 자 하나의 feature에 대해서 진행을 해보자
            tmp_sentence_mask = [0] * len(["CLS"])
            see_tokens = []
            for j in evidence_index:
                see_tokens.extend(feature.cur_sent_range[j])

            sentences = torch.tensor(feature.input_ids)[see_tokens]
            tmp_input_ids = sentences.tolist()
            tokens = tokenizer.convert_ids_to_tokens(tmp_input_ids)
            evidence_sentence = " ".join(tokens)
